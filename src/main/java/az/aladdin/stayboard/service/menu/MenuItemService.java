...
public class MenuItemService extends HotelAwareService {
    ...
    private final MenuItemAssociationService menuItemAssociationService;
    ...
    public MenuItemResponse create(CreateMenuItemRequest request) {
        ...
        menuItemAssociationService.syncAllergens(entity, request.allergenIds(), hotelId);
        menuItemAssociationService.syncDietaryTags(entity, request.dietaryTagIds(), hotelId);
        menuItemAssociationService.syncModifierGroups(entity, request.modifierGroupIds(), hotelId);
    ...
    public Page<MenuItemResponse> search(MenuItemSearchCriteria criteria, Pageable pageable) {
        Long hotelId = getCurrentHotelId();
        Page<MenuItemEntity> entityPage = menuItemRepository.findAll(
                MenuItemSpecification.withCriteria(hotelId, criteria),
                pageable
        );
        // Map inside the transaction (open-in-view is disabled).
